...
export const useRecordScreenZone = () => {
    ...
    const startRecording = () => {
        return new Promise(async (resolve, reject) => {
            const res = await RecordScreen.startRecording({ mic: false }).catch(reject);
            if (res) {
                resolve(res);
            }
        });
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (res) {
                ...
                FFmpegKit.executeAsync(args.join(' '), async (session) => {
                    ...
                    if (state === SessionState.FAILED.toString() ||
    ...
    const Wrapper = (props) => {
        return (React.createElement(View, { ...props, onLayout: onLayout }, props.children));
    };
    return {
        startRecording,
        stopRecording,
