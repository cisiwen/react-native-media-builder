...
export const SwappableViewItem = (props) => {
    ...
    const style = useAnimatedStyle(() => {
        ...
        return {
            padding: scale.value,
            opacity: (_a = props.item.style.hitTestOpacity) === null || _a === void 0 ? void 0 : _a.value,
        };
    });
        ...
        for (let i = 0; i < props.allItems.length; i++) {
            ...
            if (item.id !== props.item.id) {
                ...
                if (item.layout) {
                    ...
                    let xMeeted = evt.nativeEvent.absoluteX > value.pageX &&
                        evt.nativeEvent.absoluteX < x2;
                    let yMeeted = evt.nativeEvent.absoluteY > value.pageY &&
        ...
        if (!props.moveOnly) {
            ...
            if (hitTarget) {
                if (hitTarget.setUrlState && props.item.setUrlState) {
                    let targetUrl = (_a = hitTarget.uriShareValue) === null || _a === void 0 ? void 0 : _a.value;
                    if (targetUrl) {
                        let myUrl = (_b = props.item.uriShareValue) === null || _b === void 0 ? void 0 : _b.value;
                        if (myUrl) {
    ...
    return (React.createElement(TapGestureHandler, { onHandlerStateChange: handler },
        React.createElement(Animated.View, { ref: props.item.element, style: [UIStyle.flexOne] },
            React.createElement(LongPressGestureHandler, { onHandlerStateChange: onHandlerStateChange, onEnded: onDragEnded, maxDist: 10050, onGestureEvent: onMoveGestureEvent },
                React.createElement(View, { style: [UIStyle.flexOne] },
                    React.createElement(Animated.View, { style: [UIStyle.flexOne, style] }, props.children))))));
};
