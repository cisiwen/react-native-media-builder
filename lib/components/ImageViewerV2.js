...
export const ImageViewerV2 = (props) => {
    ...
    const renderAnimateImage = () => {
        return (React.createElement(Animated.Image, { onLayout: onImageLayoutChange, onLoadStart: () => {
            ...
            }, style: [animatedStyles], source: { uri: props.url } }));
    };
    const renderTestImage = () => {
        return (React.createElement(Image, { style: [styles.image], onError: (error) => {
                console.log(error.nativeEvent.error);
            }, source: { uri: props.url } }));
    };
    console.log('ImageViewerV2', props.url);
    return (React.createElement(PanGestureHandler, { maxPointers: 1, onGestureEvent: onPanGestureHandlerV2 },
        React.createElement(Animated.View, { onLayout: (evt) => {
                onLayoutChange(evt);
            }, style: [styles.rootContainer] },
            React.createElement(View, { style: [styles.flexOne] },
                React.createElement(PinchGestureHandler, { onGestureEvent: onPinchGestureHandler },
                    React.createElement(Animated.View, { style: [styles.container], collapsable: false }, props.itemRender
                        ? props.itemRender(props, onImageLoaded, animatedStyles)
                        : renderAnimateImage()))))));
};
