...
import type { ICollageLayout } from 'react-native-media-builder';
import {
...
export function CollageLayoutScreen(props: ICollageLayoutScreenProps) {
  ...
  const renderVideo = (
    ...
    return (
      <Animated.View
        style={[c, { flex: 1, width: '100%', backgroundColor: '#000' }]}
      >
        <Video
  ...
  const renderItems = (
    hostProps: IImageViewerV2Props,
    b: OnItemRenderItemLoaded,
    c: StyleProp<Animated.AnimateStyle<StyleProp<ImageStyle>>>
  ) => {
    ...
    React.useEffect(() => {
      console.log(
        `renderItems-${hostProps.sourceData.id}-${isVideo}`,
        hostProps.url
      );
    }, [hostProps, isVideo]);
    return isVideo
      ? renderVideo(hostProps.url, b, c)
      : renderAnimateImage(hostProps.url, b, c);
  };
